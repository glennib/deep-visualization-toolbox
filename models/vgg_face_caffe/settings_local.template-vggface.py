...
caffevis_caffe_root      = '/home/glenn/caffe'

# Load model: bvlc-googlenet
# Path to caffe deploy prototxt file. Minibatch size should be 1.
caffevis_deploy_prototxt = '%DVT_ROOT%/models/vgg_face_caffe/VGG_FACE_deploy.prototxt'

# Path to network weights to load.
caffevis_network_weights = '%DVT_ROOT%/models/vgg_face_caffe/VGG_FACE.caffemodel'

# Other optional settings; see complete documentation for each in settings.py.
caffevis_data_mean       = (129.1863,104.7624,93.5940)   # per-channel mean
caffevis_labels          = '%DVT_ROOT%/models/vgg_face_caffe/names.txt'
caffevis_jpgvis_layers   = []
caffevis_prob_layer      = 'prob'
caffevis_label_layers    = None # ('loss3/classifier', 'prob')
...
# Images are by default loaded with channel order RGB.
# If order needs to be changed to e.g. BGR, set this 
# variable to [2,1,0].
caffevis_channel_order = [2,1,0]